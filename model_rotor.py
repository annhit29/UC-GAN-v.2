'''
Difference between model.py and model_rotor.py: 
change every 26 to 32 because of Lorenz cipher that can encode to /394+8
'''
import torch
...
class Multiply(nn.Module):
    def __init__(self):
        ...
        self.embed = nn.parameter.Parameter(
            torch.FloatTensor(256, 32), requires_grad=True)
    ...
    def forward(self, sim):
        ...
        # (batch, 28, 100) => x[0] : (batch, 32, 100) and x[1] : (batch, 2, 100)
        x = torch.split(sim, 32, dim=1)
...
class Generator(nn.Module):
    ...
    def __init__(self, out_dim=32, c_dim=2): #these default values will be overwritten (cf L98 & 99), so no effects here
        ...
        # Decoding
        # output will be (64, 32, 100)
        layers.append(nn.Conv1d(out_dim, 32, 1, 1, 0))
        layers.append(nn.LayerNorm([32, 100], elementwise_affine=True))
        layers.append(nn.ReLU(inplace=True))
