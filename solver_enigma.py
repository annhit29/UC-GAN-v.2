# %%writefile /kaggle/working/UC-GAN-v.2/lib/solver_enigma.py
...
def reset_enigma_machine(engine):
    """
...
"""Enigma machine class, coz goal: Test on single ciphertext or w/ 3 CTS from the same cipher."""
class Solver_Enigma(object):
    ...
    def train(self):
        ...
        accu_tmp = []   # for Enigma CT1
        accu_tmp2 = []  # for Enigma CT2
        accu_tmp3 = []  # for Enigma CT3

        # accu test from PT to CT
        accu_tmp01 = []  # Enigma CT1
        accu_tmp02 = []  # Enigma CT2
        accu_tmp03 = []  # Enigma CT3

        # accu test for each Enigma cipher's CT emulation
        accu_CtoV = []
        accu_CtoS = []
        accu_VtoC = []
        accu_VtoS = []
        ...
        for i in range(start_iters, self.num_iters, 10000):
            ...
            accu = 0
            while e < len(id1):
                list4 = ''  # for idx(Enigma CT1) <- store characters from the "real" ciphertext x_fixed_total_test
                list5 = ''  # for recovered PT from idx(Enigma CT1) <- store characters from the generated plaintext (so fake plaintext) 
                ...
                for q in range(100):
                    for w in range(26):
                        if (x_fixed_total_test[id1[e]][w][q].item() == 1.):
                            list4 += (chr(97+w))
                            # print("list4 = Enigma CT1 =", list4)
                        if (x_fixed_fake_test[id1[e]][w][q].item() == 1.):
                            list5 += (chr(97+w))
                
                # Decrypt line using Enigma cipher:
                reset_enigma_machine(engine) #before decryptying an enigma CT
                last = engine.encipher(list4) # the plaintext obtained after decryption list4
