...
#----files for EnigmaM4 and TypeX machine:----
from lib.typex_machine.enigma_typex import *
...
#----Set up the TypeX machine----
t_pb = ""  # Set your plugboard settings here if needed
t_rotors_5 = "TYPEX_A TYPEX_B TYPEX_C TYPEX_D TYPEX_E"  # 5 TypeX rotors
...
def typex_encrypt_or_decrypt(inputMsg):
    #1. change to uppercase letters (<- the curr only valid alphabets):
    inputMsg = str(inputMsg).upper() #this typex only receives uppercase letters 
    #must always reset the machine before each msg encryption/decryption (step2.~4.):
    #2. Create a Plugboard instance
    plugboard = Plugboard([PlugLead(mapping) for mapping in t_pb.split()])
    #3. Create a MultiRotor instance for TypeX
    multirotor = MultiRotor(t_rotors_5, t_reflector, t_ring_settings, t_initial_positions)
...
"""Rotor machine class"""
class Solver_Rotor_Enigma_Typex(object):
    ...
    def Simplex(self, x):   # x : (batch_size, 1, sample * vocab_size)
        ...
        for q in range(x.size(0)):  # x.size(0) = batchsize
            ...
            for i in range(x.size(2)): # x.size(2) = 100
                ...
                for j in range(len(arr)):
                    ...
                    #     print("tour ", j)
                    #     if (j==31):
                    #         print("round(float(x[q][0][i]), 6) = ", round(float(x[q][0][i]), 6))
                    #         print("round(float(arr[j]), 6) = ", round(float(arr[j]), 6))

            # print(i)
            simplex = torch.from_numpy(np.array(simplex, dtype=np.float32))
            # print("simplex.shape is ", simplex.shape)
            x_total = torch.cat((x_total, simplex))
            # print("x_total.shape is ", x_total.shape) # torch.Size([0])
        # print("x_total is", x_total)
    ...
    def train(self):
        ...
        accu_tmp = []   # for Enigma
        accu_tmp2 = []  # for Enigma M4
        accu_tmp3 = []  # for TypeX

        # accu test from PT to CT
        accu_tmp01 = []  # Enigma
        accu_tmp02 = []  # Enigma M4
        accu_tmp03 = []  # TypeX
        ...
        for i in range(start_iters, self.num_iters, 10000):
            ...
            accu = 0
            while e < len(id1):
                list4 = ''  # for idx(Enigma CT)
                list5 = ''  # for recovered PT from idx(Enigma CT)
                ...
                last = enigma_encrypt_or_decrypt(list4)  # for recovered Enigma CT
            ...
            # Enigma decryption done

            e = 0
            accu2 = 0
            while e < len(id2):
                list6 = ''  # for idx2(Enigma M4 CT)
                list7 = ''  # for recovered PT from idx2(Enigma M4 CT)
                ...
                # Decrypt line
                last2 = enigmaM4_encrypt_or_decrypt(list6) # for recovered Enigma M4 CT
            ...
            print("Enigma to plain : ", accu)
            print("Enigma M4 to plain : ", accu2)
            ...
            # Target domain is (tt+1) (1 : Enigma, 2 : Enigma M4, 3 : TypeX)
            for tt in range(3):
                x_fixed_fake_test = self.G(
                ...
                while e < len(id0):
                    ...
                    elif (tt+1) == 2:  # Enigma M4
                        last = enigmaM4_encrypt_or_decrypt(list4)
                        # Enigma M4 done
                ...
                if (tt+1) == 1:
                    ...
                    print("plain to Enigma : ", accu1)
                elif (tt+1) == 2:
                    accu2 = accu2 / len(id0)
                    accu_tmp02.append(accu2)
                    print("plain to Enigma M4 : ", accu2)
                else:
                    accu3 = accu3 / len(id0)
                    accu_tmp03.append(accu3)
                    print("plain to Enigma : ", accu3)
            ...
            # @@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
            # Target domain is (0 : plain, 2 : Enigma M4, 3 : TypeX)
            for tt in range(4):
                if tt == 1:
                    continue
                ...
                while e < len(id1):
                    ...
                    #decrypt to PT using Enigma
                    list4 = enigma_encrypt_or_decrypt(list4)

                    if tt == 1 or tt == 0:  # Enigma or plain
                        e += 1
                        continue

                    elif (tt) == 2:  # Enigma M4
                        # encrypt to Enigma M4
                        last = enigmaM4_encrypt_or_decrypt(list4)
                        #Enigma done
                ...
                if tt == 1 or tt == 0:
                    ...
                    print("Enigma to Enigma M4 : ", accu2)
                else:
                    accu3 = accu3 / len(id1)
            ...
            # @@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
            # Target domain is (0 : plain, 1 : Enigma, 3 : TypeX)
            for tt in range(4):
                if tt == 2:
                    continue
                ...
                if tt == 2 or tt == 0:
                    ...
                    print("Enigma M4 to Enigma : ", accu2)
                else:
                    accu3 = accu3 / len(id2)
                    accu_VtoS.append(accu3)
                    print("Enigma M4 to TypeX : ", accu3)

            # @@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
            # Target domain is (0 : plain, 1 : Enigma, 2 : Enigma M4)
            for tt in range(4):
                if tt == 3:
                    continue
                ...
                if tt == 3 or tt == 0:
                    ...
                    print("TypeX to Enigma : ", accu2)
                else:
                    accu3 = accu3 / len(id3)
                    accu_StoV.append(accu3)
                    print("TypeX to Enigma M4 : ", accu3)
